...
that satisfies the maximum number of clauses---this problem is called Max\SAT{}
...
infinite) cost for failing to satisfy each clause and seeks to minise total
cost.
...
In a declarative programming language, one describes \emph{what} is to be
computed but not \emph{how}. Here we describe two declarative programming
paradigms pertinent to our work: logic programming and constraint programming.
...
% TODO (V): Is it worth clarifying that a clause can also mean a disjunction of
% literals generally, but A specific format is required in logic programming
