...
% TODO (V) (first paragraph): This is redundant right given all the text you have written previously. Perhaps simply position the exact focus of the current chapter. You also need to contrast it to the contributions of the previous chapter. Or at least say given what you explored in the previous chap what is it that you will do with the current chapter. I don't see a discussion of this.
...
% \citep{DBLP:conf/focs/BacchusDP03,DBLP:journals/jda/SamerS10}, this can make WMC
% unnecessarily slow and could be detrimental to WMC algorithms such as
% \textsf{ADDMC} \citep{DBLP:conf/aaai/DudekPV20} that depend on variable ordering
% heuristics.
...
\section{WMC} % TODO: probably need to rewrite this

 % TODO (V): Perhaps you need to quickly argue why in different chapters you have different perspectives on the same formalism. You could argue that these different perspectives is precisely what allows you to explore different types of generalisations
...
the authors and the year of publication. \Citet{DBLP:conf/kr/Darwiche02} was the first to suggest the
...
acts simultaneously as a parameter variable. \Citet{DBLP:conf/ijcai/ChaviraD05} propose
...
\begin{definition}[PBP Instance] \label{def:pbp}
  ...
  $\omega \in \mathbb{R}$ is the scaling factor.\footnote{
    ...
    \citep{DBLP:conf/ecai/BartKLM16}.} Its \emph{answer} is $\omega \cdot
...
\begin{proof}
  ...
  \end{align*}
   ...
   \]
   which is exactly what we needed to show. This ends the proof that the first loop of \cref{alg:transformation} preserves the answer under both \cref{cond:d02} and \cref{cond:bklm16}. Finally, the loop on \crefrange{line:foreach2start}{line:foreach2end} of \cref{alg:transformation} replaces $[v]_1^p[\neg v]_1^q$ with $[v]_q^p$ (for some $v \in X_I$ and $p, q \in \mathbb{R}$), but, of course, $[v]_1^p[\neg v]_1^q = [v]_1^p[v]_q^1 = [v]_q^p$, i.e., the answer is unchanged.
\end{proof}
...
\begin{figure}
  ...
  \begin{minipage}{0.49\textwidth}
    ...
    \begin{tabular}{lrr}
      ...
      \bottomrule
    \end{tabular}
    \captionof{table}{The numbers of instances (out of 1466) that each algorithm
      and encoding combination solved faster than any other combination and in
      total.}\label{tbl:performance}
  \end{minipage}
\end{figure}

\Cref{fig:cumulative2} shows $\textsf{DPMC}+\texttt{bklm16++}$ to be the
...
% TODO (V): again make sure that the conclusions are befitting a thesis by indicating pointers to previous and upcoming chapters. This should be a minor modification but is still necessary. You could search for the text "in this paper" and this will tell you where the changes need to occur

% TODO (V): I also think this conclusion is too short at least for the thesis. You need to have more reflection on what was accomplished and where things are. For example if you focused on bayes networks does that mean that an important direction for the future is to look at other kinds of models? What are the directions for the future present themselves that you have not investigated in this thesis? What other lessons can you draw? These commands are applicable in general to all conclusions of chapters.
