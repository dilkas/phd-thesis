...
of Bayesian networks to SDDs \citep{DBLP:conf/ecsqaru/ChoiKD13} and
probabilistic SDDs \citep{DBLP:conf/nips/ShenCD16} have been investigated.
However, SDDs only support weights on literals, and so are not expressive enough
to avoid the issue. \Citet{DBLP:conf/uai/GogateD10} propose a format based on
weighted clauses and probabilistic semantics inspired by Markov networks.
However, with a new representation comes the need to invent new encodings and
inference algorithms. Moreover, to the best of our knowledge, none of these
approaches
\citep{DBLP:conf/ecsqaru/ChoiKD13,DBLP:conf/uai/GogateD10,DBLP:conf/nips/ShenCD16}
...
\begin{algorithm}[t]
  ...
  \KwIn{WMC (or minimum-cardinality WMC) instance $(\phi, X_I, X_P, w)$}
  \KwOut{PBP instance $(F, X_I, \omega)$}
  $F \gets \emptyset$\;
  $\omega \gets 1$\;
...
applying \cref{alg:transformation}. Hence, the observed performance improvement
is more likely due to ADDs being smaller as a result of there being fewer
variables.\footnote{The data on this (along with the implementation of
