...
\todo[inline]{Continue writing below. Merge the two paragraphs.

  Position the exact focus of the current chapter. You also need to contrast it
  to the contributions of the previous chapter. Or at least say given what you
  explored in the previous chap what is it that you will do with the current
  chapter. I don't see a discussion of this. }
...
\todo[inline]{Probably need to rewrite this.}

\todo[inline]{Perhaps you need to quickly argue why in different chapters you
  have different perspectives on the same formalism. You could argue that these
  different perspectives is precisely what allows you to explore different types
  of generalisations}
...
\begin{definition}[Operations] \label{def:operations} Let
...
\begin{algorithm}[t]
  ...
  \ForEach{clause $c \in \phi$\label{line:foreach1start}}{
    \uIf{$c \cap X_P = \{\, p \,\}$ for some $p$ \textnormal{\textbf{and}} $w(p) \ne 1$}{
      ...
      \Else{
        $F \gets F \cup \left\{\, {\left[ \bigwedge_{l \in c \setminus \{\, p \,\}} \neg l \right]}^{w(p)}_1 \,\right\}$\;
      }
    }
...
\todo[inline]{Connect this to the previous chapter?}
...
\todo[inline]{ I also think this conclusion is too short at least for the
  thesis. You need to have more reflection on what was accomplished and where
  things are. For example if you focused on bayes networks does that mean that
  an important direction for the future is to look at other kinds of models?
  What are the directions for the future present themselves that you have not
