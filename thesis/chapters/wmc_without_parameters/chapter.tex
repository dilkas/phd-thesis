...
% TODO (V) (first paragraph): This is redundant right given all the text you
% have written previously. Perhaps simply position the exact focus of the
% current chapter. You also need to contrast it to the contributions of the
% previous chapter. Or at least say given what you explored in the previous chap
% what is it that you will do with the current chapter. I don't see a discussion
% of this.
...
the authors and the year of publication. \Citet{DBLP:conf/kr/Darwiche02} was the
...
acts simultaneously as a parameter variable. \Citet{DBLP:conf/ijcai/ChaviraD05}
...
indicator variables).\footnote{\Cref{example:2} demonstrates what we mean by
  ...
  \begin{enumerate}
    ...
    \item or for all $p \in X_P$,\label{cond:bklm16}
          \begin{enumerate}
            ...
            \item if $\{ p \} \in \phi$, then there is no clause $c \in \phi$
                  such that $c \ne \{ p \}$ and
...
% TODO: connect this to the previous chapter?
...
% TODO (V): I also think this conclusion is too short at least for the thesis.
% You need to have more reflection on what was accomplished and where things
% are. For example if you focused on bayes networks does that mean that an
% important direction for the future is to look at other kinds of models? What
% are the directions for the future present themselves that you have not
% investigated in this thesis? What other lessons can you draw? These commands
% are applicable in general to all conclusions of chapters.
