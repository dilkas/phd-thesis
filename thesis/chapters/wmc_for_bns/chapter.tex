...
\textsc{ADDMC} \citep{DBLP:conf/aaai/DudekPV20} is conceptually similar to
...
\begin{example}[Relation to the classical (logic-based) view of WMC]
...
\begin{algorithm}[t]
  ...
  \KwIn{set of random variables (i.e., nodes) $\mathcal{V}$, probability distribution $\Pr$}
  \KwOut{pseudo-Boolean function $\phi\colon 2^U \to \mathbb{R}_{\ge 0}$}
  $\phi \gets 1$\;
  \ForEach{random variable $X \in \mathcal{V}$}{
    \textit{let} $\mathrm{pa}(X) = \{\, Y_1, \dots, Y_n \,\}$\;
    $\mathrm{CPT}_X \gets 0$\;
    \eIf{$|\Imm X| = 2$}{
      \ForAll{values ${(y_i)}_{i=1}^n \in \prod_{i = 1}^n \Imm Y_i$}{
        $p_1 \gets \Pr(X = 1 \mid y_1, \dots, y_n)$\;
        $p_0 \gets \Pr(X \ne 1 \mid y_1, \dots, y_n)$\;
        $\mathrm{CPT}_X \gets \mathrm{CPT}_X + p_1[\lambda_{X=1}] \cdot \prod_{i=1}^n [\lambda_{Y_i=y_i}] + p_0 \overline{[\lambda_{X=1}]} \cdot \prod_{i=1}^n [\lambda_{Y_i=y_i}]$\;
      }
    }{
      \textit{let} $\Imm X = \{\, x_1, \dots, x_m \,\}$\;
      \ForAll{values $x \in \Imm X$ {\rm \textbf{and}} ${(y_i)}_{i=1}^n \in \prod_{i = 1}^n \Imm Y_i$}{
        $p_x \gets \Pr(X = x \mid y_1, \dots, y_n)$\;
        $\mathrm{CPT}_X \gets \mathrm{CPT}_X + p_x[\lambda_{X=x}] \cdot \prod_{i=1}^n [\lambda_{Y_i=y_i}] + \overline{[\lambda_{X=x}]} \cdot \prod_{i=1}^n [\lambda_{Y_i=y_i}]$\;
      }
      $\mathrm{CPT}_X \gets \mathrm{CPT}_X \cdot \left( \sum_{i=1}^m [\lambda_{X = x_i}] \right) \cdot \prod_{i=1}^m \prod_{j=i+1}^m (\overline{[\lambda_{X = x_i}]} + \overline{[\lambda_{X = x_j}]})$\;
    }
    $\phi \gets \phi \cdot \mathrm{CPT}_X$\;
  }
\end{algorithm}

\Cref{alg:encoding} shows how a Bayesian network with nodes $\mathcal{V}$ can be
...
In the original format, a line such as
\[
  \begin{array}{lrrll}
    w &1 &0.3
  \end{array}
...
\textsc{Ace} was not used to encode evidence, as preliminary experiments
...
\begin{figure}[t]
  \centering
  \input{chapters/wmc_for_bns/cumulative.tex}%
...
\begin{figure}[t]
  \centering
  \input{chapters/wmc_for_bns/scatter.tex}%
...
We provide two plausible explanations for why \textsc{ADDMC}
...
number of values per variable. \Cref{tbl:asymptotes} shows how \texttt{cw} has
...
\Cref{tbl:asymptotes} also shows that \texttt{cw} has asymptotically fewer ADDs
...
\Cref{chapter:wmc2} continues to develop the perspective on WMC initiated in
