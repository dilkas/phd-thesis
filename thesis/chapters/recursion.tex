...
closely. The FCG in \cref{fig:examplefcg,example:interpretation} counts
injections and is responsible for the $mn$ entry in the table.\footnote{A very
...
$f(0, 0) = 1$, and $f(m, 0) = 0$ for all $m > 0$. In other words, these bases
state that an injection to an empty codomain exists if and only if the domain is
empty. Finally, note that $f(m, n)$ can be evaluated in $\Theta(mn)$ time by a
...
% Correctness/efficiency guarantees?

% \begin{itemize}
%   \item Explain why these solutions are always computable in polynomial time. A
%         finite FCG can introduce only a finite amount of new domains (whose
%         sizes are upper bounded by the input domains) and a finite amount of new
%         functions. Everything can be computed in polynomial time by DP (but
%         sometimes not computing some of entries might be faster) as long as it
%         terminates at all.
...
%         domain sizes could be useful because these problematic terms might
%         cancel out. A realistic example based on the solutions that
%         \textsc{Crane} constructed for the instances in \cref{sec:results} could
%         look like
%         \[
%           f(n) = -f(n) + \sum_{m=0}^{n} \binom{n}{m} f(m).
%         \]
