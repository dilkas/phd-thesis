...
\todo[inline,caption={}]{
  \begin{itemize}
    ...
    \item Where do I mention, e.g., the out-degree of each type?
  \end{itemize}
}
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[every node/.style={draw,ellipse},edge from
    parent/.style={draw,-latex},sibling distance=2cm,level 6/.style={sibling
      distance=6cm}]
    \node (dr) {$\GDR_{a \gets a \setminus \{\, x \,\}}$} child {node
...
where $G$ is an FCG, and $L$ is a list of formulas such that $|L|$ is equal to
the number of $\star$'s in $G$. Let the \emph{underlying tree} of an FCG $G$ be
the induced subgraph of $G$ that omits all $\Reff$ nodes (subsequently presented
algorithms ensure that it is always a tree). Then we can define an implicit
...
underlying tree of $G$. For example, if the FCG in \cref{fig:ordering} were to
be returned by a compilation rule, its corresponding list of formulas $L$ would
have two elements. These elements would then be implicitly associated with the
$\star$'s that have labels `3' and `4' next to them, respectively.

\todo[inline]{if a rule is not applicable to a given formula, it returns an
  empty set and is skipped.}

\todo[inline]{This explanation should be positioned independently of
  compilation rules.}
...
\begin{algorithm}
  \caption{The compilation rule for $\GDR$ nodes}
...
\todo[inline,caption={}]{
  \begin{itemize}
    \item Intersperse algorithms and their descriptions.
    \item Could \texttt{mergeFcgs} be rewritten to work on pairs of graphs and
          lists?
  \end{itemize}
}
...
\todo[inline]{Will need to update this last sentence to point to the new
  definition.}
...
\begin{algorithm}
  \caption{The function for applying non-greedy rules.}
  ...
  \Fn{\applyAllRules{state $s$}}{ $(G, C, L) \gets s$\;
    ...
    \ForEach{non-greedy rule $r$}{ \ForEach{$(G'', L'') \in r(\phi)$}{
        \lIf{$G'' = \star$}{\KwRet{\applyAllRules{$(G', C', L'' \mdoubleplus T)$}}}\label{line:nullfcg}
...
\begin{algorithm}
  ...
  \Fn{\applyGreedyRules{formula $\phi$, cache $C$}}{ \ForEach{greedy rule $r$
      ...
      } r(\phi)$\; \lIf{$G = \star$}{\Return{\applyGreedyRules{the element of
            $L$, $C$}}} \Return{\applyGreedyRulesToAllFormulas{$G$,
          \updateCache{$C$, $\phi$, $G$}, $L$}}\; }
    \Return{$(\star, C, \langle\phi\rangle)$}\; }
  \Fn{\applyGreedyRulesToAllFormulas{$(V, s, N^+, \tau)$, cache $C$, list $L$}}{
    ...
    $L' \gets \langle\rangle$; $N^+(s) \gets \langle\rangle$\; \ForEach{formula
      ...
      \uIf{$G' = \star$}{
        add a new node $t$ to $V$ with $\tau(t) = \star$ and $N^+(t) = \langle\rangle$\;
        $N^+(s) \gets N^+(s) \mdoubleplus \langle t \rangle$\;
      }
...
\begin{algorithm}
  ...
  \Fn{\updateCache{cache $C$, formula $\phi$, FCG $(V, s, N^+, \tau)$}}{
    ...
    \ForEach{$t \in N^+(r)$}{
      \If{$\tau(t) = \star$}{ replace $t$ with $s'$ in
...
\todo[inline]{Describe the two remaining algorithms:}
\begin{itemize}
  \item \cref{alg:apply}
        \begin{itemize}
