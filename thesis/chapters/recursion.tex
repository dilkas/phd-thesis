...
\subsubsection{A Generalisation of Domain Recursion} \label{sec:dr}
...
\todo[inline]{Invite the reader to look up the original paper for more details.}
...
\end{itemize}
We replace them with just one condition featured on \cref{line:condition} of \cref{alg:domainrecursion}: for domain recursion to be applicable on domain $d \in \mathcal{D}$, there must be at least one variable \emph{featured in a literal} associated with $d$. This condition is necessary because domain recursion on $d$ (both this new version and the original one) considers all possible ways of replacing a subset of the variables in literals with a constant. Hence, if there are no such variables,

\todo[inline]{Describe \cref{alg:domainrecursion}. Compare with the original \citep{DBLP:conf/nips/Broeck11}.}
\paragraph{Originally...}
\begin{itemize}
\item Three conditions (verbatim):
\begin{itemize}
\item the formula is shattered (i.e., all pairs of atoms represent either a disjoint or identical set of ground atoms)
\item the formula cannot be partitioned into independent subformulas that could be solved separately
\item there exists a root binding class (of variables) (see the original paper for the definition)
\end{itemize}
\item We replace these conditions with just one. The reason for this precondtion is the same as in the initial version of domain recursion: there must be a variable with that domain featured among the literals because it needs to be replaced by a constant.
