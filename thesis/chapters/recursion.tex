...
\todo[inline]{use some other (non-slanted) font for names of fragments}

\begin{itemize}
  \item all formulas with up to two variables (i.e., those in $FO^{2}$) are
        known to be domain-liftable \citep{DBLP:conf/nips/Broeck11}
  \item there exists a formula with three variables (i.e., in $FO^{3}$) for
...
\begin{itemize}
  \item focus on specific fragments of logic
        \begin{itemize}
          \item $FO^{2}$ (a faster result than the proof by Beame
                et al.) \citep{DBLP:conf/uai/BremenK21}
          \item new liftable classes using domain recursion ($S^{2}FO^{2}$ and
                $S^{2}RU$) \citep{DBLP:conf/nips/KazemiKBP16}
          \item new liftable classes ($U_{1}$, less significant than counting
                quantifiers) \citep{DBLP:conf/lics/KuusistoL18}
          \item $C^{2}$: two variable fragment with counting quantifiers
...
% TODO: start here. Decide on what I want to mention about each paper and in what order.
