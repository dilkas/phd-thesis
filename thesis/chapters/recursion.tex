...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[every node/.style={draw,ellipse},edge from parent/.style={draw,-latex},sibling distance=2cm,level 6/.style={sibling distance=6cm}]
    ...
    child {node {$\bigvee_{b^\top \subseteq b}$}
      child {node[circle] {$\wedge$}
        child {node[fill=green!10,circle] {$1$}} % _{(\{\, p(x, X) \,\}, \emptyset, \{\, X \mapsto b^\top \,\})}
        child {node[circle] {$\wedge$}
          child {node[fill=green!10,circle] {$1$}} % _{(\{\, \neg p(x, X) \,\}, \emptyset, \{\, X \mapsto b^\bot \,\})}
          child {node[circle] {$\wedge$}
  ...
  \end{tikzpicture}
  \caption{An example FCG constructed by Crane. Some parameters are omitted.}
  \label{fig:examplefcg}
\end{figure}

\begin{example}
TODO: describe \cref{fig:examplefcg}. Connect it to the definition (e.g., labels, edge ordering, source).
\end{example}
...
\todo[inline]{Clearly explain what we mean by `encodes'.}
