...
% TODO (after 'construct recursive solutions'): Might be good at this point to
% enumerate a few examples of problems that can be handled by the contribution
...
% TODO: Maybe explain somewhere that when lifting is not possible, classical WMC
% is computed. Also explain that under the assumption that the weights of the
% predicates is the same across all instances WFOMC is the same as WMC.
...
these graphs represent recursive calls. See \cref{fig:intuition} for an
illustration of an example scenario. Suppose the original formula $\phi$ depends
on a domain of size $n \in \mathbb{N}$. Our generalised version of domain
recursion transforms $\phi$ into a different formula $\psi$ that depends on a
domain of size $n-1$. After some number of subsequent transformations, the
...
cycle-forming edge to the graph (highlighted in blue in \cref{fig:after}), which
can be interpreted as function $f$ relying on $f(n-1)$ to compute $f(n)$.
...
rules should be applied. In \cref{sec:interpret}, we informally discuss how a
graph can be interpreted as a collection of (potentially recursive) functions.
Finally, in \cref{sec:results} we compare \textsc{ForcLift} and \textsc{Crane}
on a range of function-counting problems. We show that \textsc{Crane} performs
as well as \textsc{ForcLift} on the instances that were already solvable by
\textsc{ForcLift} but is also able to handle most instances that
\textsc{ForcLift} fails on as well.
% TODO: the GDR part is unclear
...
% TODO: explain \mathcal{D} in a bit more detail
...
\begin{algorithm}[t]
  ...
  \ForEach{domain $\Omega \in \mathcal{D}$ and element $x \in \Omega$ s.t. $x$ does not occur in any literal of any clause of $\phi$ {\bf and} for each clause $c = (L, C, \delta_c) \in \phi$ and variable $X \in \Vars(c)$, either $\delta_c(X) \ne \Omega$ or $(X, x) \in C$\label{line:crconditions}}{
    ...
    \ForEach{clause $(L, C, \delta) \in \phi$}{
      ...
      \nosemic$\delta' \gets v \mapsto
      \begin{cases}
        \Omega' & \text{if } \delta(v) = \Omega\\
...
\begin{example}
  ...
  for constraint removal. The rule introduces a new domain $\Gamma'$ and
  transforms $\phi$ to $\phi' = (c_1', c_2', c_3')$, where
  \begin{align*}
    c_1' &= c_1, \\
...
\begin{example}\label{example:interpretation}
  ...
  and $\Delta^{\bot}$, respectively. Domain $\Gamma'$ was created by a
  constraint removal rule applied on $\Gamma$, so $|\Gamma'| = m - 1$. Now
...
% TODO: It is hard to say for sure but I think the examiners as well as the
% readers of a potential paper will complain how the technical section ends. You
% need to find a proposition or in the worst case at least a conjecture that
% says that crane does something provably better or for certain compared to
% previous work. There are some papers in statistical rel learning but don't
% mention any formal results. But don't think given the kind of technical
% notation you have introduced it is justified to leave things without any
% claim. How about a proposition like: given a factorial function expressed in
% the following fragment of fo logic, the algorithm crane computes...
%
% You could then point out that forklift does not have such a claim
%
% If intermediate claims leading up to the final proposition can be provided for
% the other steps u introduce that much the better.
...
\[
  \forall W \in \Gamma\text{. }\forall X \in \Gamma\text{. }\forall Y \in \Delta\text{. }p(W, Y) \land p(X, Y) \implies W = X
\]
...
saw examples of instances that become liftable not just in principle
...
% 1) Found that injectivity is the main example of something that is now liftable
% 2) \Ctwo{}?
% TODO: So perhaps you don't have any concrete results but I think it is
% important to at least state using a conjecture what kind of classes you hope
% you can capture. If you don't make it explicit or even provide a concrete set
% of suggestions, you cannot expect the reader to arrive at this on their own.
...
% TODO ('what makes... one domain'): Again you cannot expect the reader to gain
% this insight by reading the technical work. You have to make this explicit.
% You might even use a paragraph separation and title it a remark to point out
% obvious mathematical properties of your construction.
