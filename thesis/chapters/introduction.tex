...
\chapter{Introduction} \label{chapter:introduction} % 5-14 pages (9 on average)

% ==================== WMC====================

% * description and example(s)

%% TODO (V): I do not know if you intended to add more things at the start, but this might be too abrupt start for most readers. 
%% I would instead say that probability theory and statistics is the foundation of modern data science, et cetera, and in this thesis we are concerned with certain approaches to formulate and compute probabilities in popular representations such as Bayes  networks. That’s one way of starting. The other way of starting is to make the argument about the need for the unification of logic and probability, perhaps this doesn’t fit well with the introduction as it is written.
...
% TODO (V): I think it is important to add an intermediate paragraph here where you say something like - readers familiar with proba theory but not logic might find this a curious choice. But as it turns out this formulation is an underlying computational representation for the problem off computing condition inference in a range of different representations from factor graphs to probl programs, and provide references. In fact in most inference competitions for example the conference U AI, this formulation often comes off at the top
...
\begin{table}
  ...
  \begin{tabular}{lll}
    ...
    \bottomrule
  \end{tabular}
  \caption{An assortment of problems that require one to compute a quantity defined as a sum of products.}\label{table:comparison}
\end{table}
...
% TODO (V): Perhaps conclude this list by saying in this thesis, we will look at precisely these aspects and propose certain contributions to w model counting that address these aspects from different perspectives. This provides a nice lead up to the next section
...
%% * Explain how the current state of research led me to work on my RQ. In other words, emphasise timeliness, i.e., why now? Because ADDMC allows for my ideas to become implementable.
...
\Cref{chapter:wmc2} builds on \cref{chapter:wmc1} and further considers WMC in its full generality, leading to the definition of PBP. Here we present an \emph{algorithm that transforms WMC instances into PBP instances} while eliminating around \SI{43}{\percent} of variables on average across various Bayesian network encodings. Moreover, we identify \emph{sufficient conditions} for such a variable removal to be possible. Our experiments show significant improvement in WMC-based Bayesian network inference. \Cref{chapter:wmc2} is published as:
\begin{displayquote}
...
% TODO (V) ('combine into a coherent picture'): Play a role in the overall development aroundmodel counting. Alt phrasing - We shareholder contributions connect to each other in relation to developments in model counting

We end the introduction with a visual description of how the topics covered in this thesis combine into a coherent picture. \Cref{fig:overview} lists some of the key concepts of this work and shows how each chapter of the thesis relates to these concepts and interactions between them. \Cref{chapter:background} covers a selection of topics related to this work and refers the reader to suitable literature for further information. \Cref{chapter:wmc1} examines the definition of WMC more closely and suggests a way to bypass it, leading to a more succinct encoding of Bayesian network probabilistic inference compatible with the ADDMC algorithm. Then \cref{chapter:wmc2} describes WMC encodings for Bayesian networks, defines PBP, shows how to transform WMC instances to PBP instances, and how the DPMC algorithm benefits from this new format. In \cref{chapter:wfomc}, we expand the capabilities of the WFOMC algorithm ForcLift to new (previously unsolvable) instances. \Cref{chapter:randomlps} describes a constraint model that can generate random (probabilistic) logic programs in the ProbLog \citep{DBLP:conf/ijcai/RaedtKT07} language---a well known use-case of WMC. \Cref{chapter:comparison} develops an algorithm for generating random WMC instances and uses it showcase some important differences in the behaviour of WMC algorithms. Finally, \cref{chapter:conclusion} summarises our results and provides a perspective for potential future work.
