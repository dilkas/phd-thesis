...
(\cref{chapter:wmc2}). Exact \textsf{WMC} algorithms can be broadly classified
as based on search \citep{DBLP:conf/sat/SangBBKP04,DBLP:conf/ijcai/SharmaRSM19},
...
we see most \textsf{WMC} algorithms performing very similarly overall but with
...
% compare \textsf{WMC} algorithms on random instances of a particular
% application of \textsf{WMC} (i.e., probabilistic logic programs), it fails to
...
almost forty years. Variations of some of the first random models ever proposed
...
\citep{DBLP:conf/focs/AchlioptasM02} and efficiently approximating $\#\SAT{}$
...
only one \textsf{WMC} algorithm so far has been analysed using random instances
\citep{DBLP:conf/sat/SangBBKP04,DBLP:conf/sat/SangBK05}. The goal of this
chapter is to explain some of the differences between \textsf{WMC} algorithms
via an experimental study that uses random instances.
...
define weights on literals.\footnote{Recall that in
  \cref{chapter:wmc1,chapter:wmc2} we showed that algorithms such as
  \textsc{DPMC} and \textsc{ADDMC}
