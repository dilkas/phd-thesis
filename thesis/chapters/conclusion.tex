...
% TODO (V): Perhaps you should start by saying recall that WMC is a fundamental computational task underlying many formalisms and application. We emphasised that in order to improve the foundations and the applicability of this formulation different kinds of generalisations were needed, eg with regards to (insert bullet point list from chapter 1, just before the start of the contribution section).

% TODO (V): Generally speaking, the contributions of this thesis can be the roughly thought of in two ways
...
% TODO (V): In summary, we have a) introduce new foundations for WMC by exploring a measure theo formulation 2) introduced new encoding schemes as appropriate for dynamic programming approaches 3) extend the first definition to account for rec. solutions 4) provided algorithms for random instance testing
...
\section{Future Directions} \label{sec:future} % emphasise the big picture
...
% TODO (V): Perhaps you should add a small note here: exploring the impact of these contributions with respect to the many applications that are being considered on WMC is also worth exploring. Eg does it lead to more formalisms that can be reduced owing to the measure theo foundations. Does it lead to more practical algorithms for ProbLog. Does it mean the computation of the loss function in the nesy approaches can be made faster, see if you can provide an informal justification for why this should be the case
...
In this thesis, we introduce two ways to generate random instances: one for (probabilistic) logic programs and one for propositional formulas in CNF that are then turned into WMC instances. As we provide the very first attempts at testing WMC algorithms on random data, many opportunities for improvements and future work remain. An interesting opportunity to connect our work on random logic programs in \cref{chapter:randomlps} and on (W)FOMC in \cref{chapter:wfomc} is by adapting the constraint model to generate (W)FOMC instances instead of logic programs. This way one could systematically search for interesting instances that, e.g.,
\begin{itemize}
\item reveal differences in the runtime complexity of various algorithms or
\item demonstrate a gap between the performance of state-of-the-art WFOMC algorithms and formulas constructed by hand.
\end{itemize}
There is also ample opportunity for theoretical contributions. For instance, one explanation for the surprising experimental results of \cref{chapter:randomlps} is that all of the generated instances yielded easy WMC problems, and the computational bottleneck was in the handling of the logic program before WMC. We can state this idea as a (somewhat informal) conjecture.
\begin{conjecture}
  With high probability, the WMC instance that results from a random probabilistic logic program generated by the constraint model in \cref{chapter:randomlps} is tractable for some WMC algorithm.
\end{conjecture}
...
% TODO (V) ('Finally, for the'): I think this reference is rather vague. Instead there has been some recent work on making connections between tractable circuits and explainability, and perhaps you can articulate something based on that. There is also some discussion on robustness in machine learning and where exact computation can help. Likewise you could reference the work on approximate counting as another dimension to your work. You could mention the use of portfolio approaches to make SAT extremely fast, and whether something similar could be tried out with your work.
