...
\todo[inline,caption={}]{Perhaps you should start by saying recall that WMC is a
  fundamental computational task underlying many formalisms and application. We
  emphasised that in order to improve the foundations and the applicability of
  this formulation different kinds of generalisations were needed, eg with
  regards to (copied from the introduction).
  \begin{itemize}
...
\todo[inline]{ In summary, we have a) introduce new foundations for WMC by
  exploring a measure theo formulation 2) introduced new encoding schemes as
  appropriate for dynamic programming approaches 3) extend the first definition
  to account for rec. solutions 4) provided algorithms for random instance
  testing}
...
algorithms based on ADDs while \textsc{ForcLift} is a WMC algorithm for
first-order logic based on knowledge compilation.

\todo[inline]{You could mention the use of portfolio approaches to make SAT
  extremely fast \citep{DBLP:journals/jair/XuHHL08} (other domains
  \citep{DBLP:conf/lion/KotthoffMS16}), and whether something similar could be
  tried out with your work.}

\todo[inline, caption={}]{Likewise you could reference the work on approximate
  counting as another dimension to your work.
  \begin{itemize}
    \item approximate \textsc{ForcLift} \citep{DBLP:conf/uai/BroeckCD12}
    \item approximations with ADDs \citep{DBLP:conf/nips/St-AubinHB00}
    \item approximate model counting \citep{DBLP:series/faia/ChakrabortyMV21}
  \end{itemize}
