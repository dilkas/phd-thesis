...
%   bodies of clauses (e.g., $\neg p(X) \lor (q(X) \land p(X))$) because ProbLog
%   does too. However, one can easily restrict our representation of a body to a
%   single conjunction of literals (e.g., $q(X) \land \neg p(X)$) by adding a
%   couple of additional constraints.}). A \emph{(logic) program} is a set of
% clauses, and a \emph{ProbLog program} is a set of clause-probability pairs
...
In \cref{sec:heads,sec:bodies}, we define clauses (i.e., their heads and bodies)
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}
    ...
    \draw[-Latex] (parent) edge node[above] {$+$} (sibling);
    \draw[-Latex] (parent) edge node[above] {$+$} (father);
    \draw[-Latex] (mother) edge node[below] {$-$} (father);
  \end{tikzpicture}%
...
\begin{algorithm}[t]
  ...
  \Fn{\getDependencies{predicate $p$, flag \all}} {
    $D \gets \{\, \Determined(p) \,\}$\;
    \While{\normalfont\true{}}{
      $D' \gets \emptyset$\;
      \ForEach{dependency $d \in D$ {\bf and} predicate $q \in \predicates{}$}{
  ...
  \begin{subfigure}[b]{.39\textwidth}
    ...
    \makebox[\linewidth]{
      \begin{tikzpicture}
        ...
        \node[draw] (sibling) at (2, 0.5) {$\mathsf{sibling}$};
        \node[draw] (father) at (2, -0.5) {$\mathsf{father}$};
        \draw[-Latex] (parent) edge (father);
        \draw[-Latex] (mother) edge (father);
        \draw[-Latex,dashed,darkgray] (parent) edge (sibling);
        \draw[-Latex,dashed,darkgray] (parent) edge (mother);
      \end{tikzpicture}
    }
...
\todo[inline]{Integrate this into the chapter. See ?? for examples of programs
  generated using various sets of parameter values. Make sure that the initial
  overview mentions this section. Modify the surrounding
  introductions/conclusions from both sides.}
...
the algorithms behaved identically. \Cref{chapter:comparison} continues the
