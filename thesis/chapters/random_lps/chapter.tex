\chapter{Generating Random Logic Programs Using Constraint
...
\todo[inline,caption={}]{
  \begin{itemize}
    \item are the definitions of fact/rule the same as in the background
          section?
    \item replace the citation for stratification in the content chapter with
          the better one from the background chapter
    \item there's no need for hyphens in 'variable-ordering' and
          'value-ordering'
    \item it's first fail (equivalent to dom) rather than fail-first
    \item make sure to mention the appendix
  \end{itemize}
}

\todo[inline]{ Here as well you seem to have used the introduction from the
  paper but somehow it seems fine here. Mainly because it's self-contained. But
  just check once more that it is connecting nicely with the previous chapters
  and you are positioning your own contribution in an effective manner. }
...
\citep{DBLP:conf/ijcai/RaedtKT07} and non-probabilistic Prolog programs.
...
the model produces for various parameter values. We end the paper with two
...
\emph{body} (which is a formula\footnote{Our model supports arbitrarily complex
  bodies of clauses (e.g., $\neg p(X) \lor (q(X) \land p(X))$) because
  \textsc{ProbLog} does too. However, one can easily restrict our representation
  of a body to a single conjunction of literals (e.g., $q(X) \land \neg p(X)$)
  by adding a couple of additional constraints.}). A \emph{(logic) program} is a
set of clauses, and a \emph{\textsc{ProbLog} program} is a set of
clause-probability pairs \citep{DBLP:journals/tplp/FierensBRSGTJR15}.
...
generating non-probabilistic programs, one can set this list to $[1]$. Finally,
we define $\tokens{} = \{\, \neg, \land, \lor, \top \,\}$ as the set of tokens
that (together with atoms) form a clause. All decision variables of the model
can now be divided into $2 \times \maxNumClauses{}$ separate groups, treating
the body and the head of each clause separately. We say that the variables are
contained in two arrays: $\arrayt{bodies}{\maxNumClauses{}}{Body}$ and
$\arrayt{heads}{\maxNumClauses{}}{Head}$.

\section{Heads of Clauses}\label{sec:heads}

\todo[inline]{ Here as well you have to motivate why you are slightly different
  from those preliminary section. But is it easy enough to do I think. All you
  have to say is that you slightly elaborate on the definitions of accommodate
  more granularity }

We define the \emph{head} of a clause as a
...
\begin{algorithm}[t]
  ...
  \lIf{$D = \emptyset$}{\Return{\normalfont\true{}}}
  \leIf{$\exists (\Determined \_, \Determined \_) \in D$}{\Return{\normalfont\false{}}}{\Return{\normalfont\undefined{}}}
...
future decisions made by the solver. \Cref{alg:independence_entailment} outlines
...
\begin{algorithm}[t]
  ...
  \For{$(d_1, d_2) \in \getDependencies{$p_1$, 0} \times \getDependencies{$p_2$,
    ...
    \lIf{$\{\, d_1, d_2 \,\} = \{\, \Determined(\_), \AlmostDetermined(\_, s, t) \,\}$\label{line:propagation_condition}}{$\mathbf{A}[s][t]$.\removeValue{$1$}}
  }
  ...
  \Fn{\getDependencies{$p$, \all}} {
    ...
    \While{\normalfont\true{}}{
      ...
      \For{$d \in D$ {\bf and} $q \in \predicates{}$}{
        ...
        \lIf{$\edgeExists$ {\bf and} $d$ {\bf is} $\Determined(\_)$}{$D' \gets D' \cup \{\, \Determined(q) \,\}$}
        \lElseIf{$\edgeExists$ {\bf and} $d$ {\bf is} $\AlmostDetermined(\_, s, t)$}{$D' \gets D' \cup \{\, \AlmostDetermined(q, s, t) \,\}$}
        \uElseIf{$|\mathbf{A}[q][d.\predicate]| > 1$ {\bf and} $d$~{\bf is}~$\Determined(r)$}{
          $D' \gets D' \cup \{\, \AlmostDetermined(q, q, r) \,\}$\;
        }
        \lElseIf{$|\mathbf{A}[q][d.\predicate]| > 1$ {\bf and} \all}{$D' \gets D' \cup \{\, \Undetermined(q) \,\}$}
      }
...
\begin{figure}[t]
  ...
  \caption{The predicate dependency graph of
    \protect{\cref{example:independence}}.}\label{fig:example}
\end{figure}
...
\begin{figure}[t]
  \centering
  \input{chapters/random_lps/impact.tex}%
...
\begin{figure}[t]
  \centering
  \input{chapters/random_lps/bars.tex}%
...
rules according to the following parameter values:
$|\predicates{}|, |\variables{}|, \maxNumNodes{} \in \{\, 2, 4, 8 \,\}$,
...
\begin{figure}[t]
  \centering
  \input{chapters/random_lps/line_plots.tex}%
